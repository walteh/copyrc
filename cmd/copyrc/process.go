	...
	if cfg.Async {
		...
		for _, file := range files {
			...
			go func(f ProviderFile) {
				defer wg.Done()
				if err := processFile(ctx, provider, cfg, f.Path, commitHash, status, mu, destPath); err != nil {
		...
		for _, file := range files {
			...
			if err := processFile(ctx, provider, cfg, file.Path, commitHash, status, mu, destPath); err != nil {
